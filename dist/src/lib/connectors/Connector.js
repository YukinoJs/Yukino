...
import { Events, SearchTypes } from '../../types/constants.js';
...
export class Connector extends EventEmitter {
    ...
    constructor(options) {
        ...
        if (!options.host && !options.url)
            throw new Error('Either host or url must be provided');
        if (!options.auth)
     ...
     */
    createNode(options, restOptions) {
     ...
     * @throws {Error} Throws if no nodes are available
     */
    async loadTrack(identifier, options = {}) {
     ...
     */
    async sendVoiceUpdate(guildId, channelId, mute = false, deaf = false) {
        const payload = {
            op: 4,
            d: {
                guild_id: guildId,

...
export class DiscordJSConnector extends Connector {
    ...
    constructor(options) {
        if (!options.client)
            throw new Error('Discord.ts Client must be provided');
     ...
     * @override
     */
    async sendVoiceUpdate(guildId, channelId, mute, deaf) {
        if (!this.client.ws)
            throw new Error('Discord.ts client WebSocket not available');
        const payload = {
            op: 4,
            d: {
                guild_id: guildId,
...
//# sourceMappingURL=DiscordJSConnector.js.map
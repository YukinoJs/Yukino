...
export class Node extends EventEmitter {
    ...
    constructor(connector, options) {
        ...
        const retryAmount = options.retryAmount ?? reconnectOptions.retryAmount ?? options.reconnectTries ?? 3;
        const retryDelay = options.retryDelay ?? reconnectOptions.retryDelay ?? options.reconnectInterval ?? 5000;
        this.options = {
            name: this.name,
            url: options.url,
            auth: this.auth,
        ...
        this.url = `${options.secure ? "wss" : "ws"}://${options.url}/v${this.options.version || Versions.WEBSOCKET_VERSION}/websocket`;
        this.rest = new Rest({
            url: options.url,
            auth: options.auth,
            secure: options.secure || false,
     ...
     */
    destroy(code = 1000, reason = "Node destroyed") {
     ...
     */
    processEventsFromResponse(response) {
        if (!response)
            return;
     ...
     */
    async callPlayerAPI(guildId, method = 'PATCH', body) {
        if (!this.sessionId)
     ...
     */
    handleEventDispatch(player, data) {
        switch (data.type) {
    ...
    scheduleReconnect() {
        ...
        this.reconnectTimeout = setTimeout(async () => {
            try {
                ...
                err.code = err.code || 'RECONNECT_FAILED';
